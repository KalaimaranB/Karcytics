...
# The DSN is read from the CI environment (GitHub Actions secret KARCYTICS_SENTRY_DSN)
# and written into a runtime hook that fires before app code in the frozen binary.
# This keeps the DSN out of source control while baking it into production builds only.
# In dev (source-tree) runs, get_configured_dsn() returns None via sys.frozen check
# so this hook is irrelevant; it only matters in the frozen exe.
_sentry_dsn = os.environ.get('KARCYTICS_SENTRY_DSN', '')
_rthook_path = os.path.join(SPECPATH, '_sentry_rthook.py')
with open(_rthook_path, 'w', encoding='utf-8') as _f:
    _f.write(
        "# Auto-generated by Karcytics.spec at build time — do not edit or commit.\n"
        "import os as _os\n"
        f"_os.environ.setdefault('KARCYTICS_SENTRY_DSN', {repr(_sentry_dsn)})\n"
    )
...
a = Analysis(
    ...
    hookspath=[],
    hooksconfig={},
    runtime_hooks=['_sentry_rthook.py'],
